...
#include <shellapi.h>
#include <d3d12.h>			// D3D12 인터페이스
#include <dxgi1_4.h>		// DXGI 팩토리 인터페이스 (스왑체인, 어댑터 열거 등등)	
#include <d3dcompiler.h>	// 쉐이더 컴파일
#include <dxgidebug.h>		// 디버그 인터페이스


// DirectXMath 수학 라이브러리 헤더 파일입니다.
...
using Microsoft::WRL::ComPtr; // ComPtr 스마트 포인터를, Com객체를 사용할 때 편하게 쓰도록 using 선언으로 추가합니다.



// 링크 지시문으로 D3D12 라이브러리를 실질적으로 링크하도록 합니다.
...
// Color 매크로 XMVector4 형식으로 변경
#define RANDOM_COLOR			XMFLOAT4(rnad() / float(RAND_MAX), \
									rnad() / float(RAND_MAX), \
									rnad() / float(RAND_MAX), 1.0f)
...
namespace Vector3
{
	inline XMFLOAT3 XAxis()
	{
		return (XMFLOAT3(1.0f, 0.0f, 0.0f));
	}
	inline XMFLOAT3 YAxis()
	{
		return (XMFLOAT3(0.0f, 1.0f, 0.0f));
	}
	inline XMFLOAT3 ZAxis()
	{
	...
	inline float Angle(FXMVECTOR& xmvVector1, FXMVECTOR& xmvVector2)
	{
	...
	inline XMFLOAT3 TransformNormal(XMFLOAT3& xmf3Vector, FXMMATRIX& xmxm4x4Transform)
	{
		XMFLOAT3 xmf3Result;
	...
	inline XMFLOAT3 TransformCoord(XMFLOAT3& xmf3Vector, FXMMATRIX& xmxm4x4Transform)
	{
		XMFLOAT3 xmf3Result;
...
namespace Vector4
{
	...
	{
		XMFLOAT4 xmf4Result;
		XMStoreFloat4(&xmf4Result, XMQuaternionIdentity());
		return(xmf4Result);
	}
